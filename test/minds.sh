...
property_m9() {
    ...
    [ "$(KHALA_HOME=$home "$KHALA" version)" = 'khala 0.9.8' ] || die "brain version is not 0.9.8"
    grep -q '"version": "0.9.8"' "$ROOT/plugin/.claude-plugin/plugin.json" ||
        die "plugin version is not 0.9.8"
